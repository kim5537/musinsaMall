...
const Navbar = ({ authenticate, setAuthenticate }) => {
  ...
  const onCheckEnter = (e) => {
    if (e.key === "Enter") navigate(`?q=${e.target.value}`); //쿼리값으로 주는 것 .쿼리는 ? 뒤에 오고 키 값을 q로 지정함.
  };
  return (
    <Wrapper>
      <HeaderTop>
        <SearchBox>
          ...
          <input type="text" placeholder="제품 검색" onKeyUp={onCheckEnter} />
        </SearchBox>
        {authenticate ? (
